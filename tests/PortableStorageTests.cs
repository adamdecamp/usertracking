...
internal static class PortableStorageTests
{
    ...
    public static int Main(string[] args)
    {
        ...
        Assert(retentionMoves.Length == 5, "Archive preflight should move disabled SAARs plus outdated active and Rework evidence before the main scan.");
        ...
        Assert(File.Exists(Path.Combine(root, "NGC", "NGC Archive", DateTime.UtcNow.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture), disabledSaarName)), "A SAAR with a standalone DISABLED filename marker should be archived before evidence validation.");
