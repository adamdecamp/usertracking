...
internal static class PortableStorageTests
{
    ...
    public static int Main(string[] args)
    {
        ...
        Assert(File.Exists(Path.Combine(reworkDirectory, currentReworkName)) && File.Exists(Path.Combine(reworkDirectory, graceReworkName)) && File.Exists(Path.Combine(reworkDirectory, currentYearOnlyName)) && File.Exists(Path.Combine(reworkDirectory, oldSaarReworkName)), "Current evidence, evidence no more than 90 days overdue, uncertain year-only evidence, and SAARs of any age must remain in Rework for filename correction.");
