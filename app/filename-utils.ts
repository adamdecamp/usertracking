...
export function filenameMatchesKind(filename:string,kind:string){
 ...
 if(canonical==='DoD Cyber Cert')return tokens.has('DOD')||tokens.has('AWARENESS')||(compact.includes('DOD')&&compact.includes('CYBER'))||(compact.includes('CYBER')&&compact.includes('AWARENESS'))||compact.includes('AWARENESSCHALLENGE');
 ...
 return (tokens.has('DTA')&&(tokens.has('TRAINING')||tokens.has('COURSE')||tokens.has('RESPONSIBILITIES')))||(compact.includes('DTA')&&(compact.includes('TRAINING')||compact.includes('COURSE')||compact.includes('RESPONSIBILITIES')));
}
