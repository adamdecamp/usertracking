...
export function reconcileEvidence(users:ReconciliationUser[],evidence:ReconciliationEvidence[],rejected:ReconciliationRejection[]){
 ...
 for(const item of evidence){const matchingUsers=users.filter(user=>filenameIdentityMatches(item.filename,user));if(!matchingUsers.length){issues.push({id:`orphan:${item.path}`,category:'Orphan Evidence',severity:'Medium',summary:`No user record matches ${item.filename}`,detail:'The file is valid evidence but its filename identity is not present in the manifest.',path:item.path});continue}for(const user of matchingUsers){const organization=organizationFrom(item.filename);if(organization&&organization.toUpperCase()!==user.organization.toUpperCase())issues.push({id:`organization:${user.id}:${item.path}`,category:'Organization Conflict',severity:'Medium',summary:`${user.last}, ${user.first}: filename organization is ${organization}`,detail:`User record organization is ${user.organization}.`,path:item.path,userId:user.id});const known=user.artifacts.some(artifact=>artifact.filename.toUpperCase()===item.filename.toUpperCase()||filenameMatchesKind(item.filename,artifact.kind));if(!known)issues.push({id:`orphan-kind:${user.id}:${item.path}`,category:'Orphan Evidence',severity:'Medium',summary:`${item.filename} is not represented in the user record`,detail:`The filename matches ${user.last}, ${user.first}, but not a tracked artifact.`,path:item.path,userId:user.id})}}
