...
- Never starts Sync automatically on launch, folder mapping, or information-system selection. The Sync scope chooser can scan the entire mapped system or exactly one immediate folder beneath `Organizations`; targeted organization Sync includes that organization's evidence tree but offers no document-type or deeper subfolder scope.
...
- Uses a checksum-protected Sync index in each mapped system folder to skip reopening unchanged evidence. The evidence-validation cache version is independent from ordinary application releases, unchanged evidence avoids per-file recovery-journal writes, and one full UTC-daily retention sweep per selected scope is followed by changed-file and Rework preflight checks. New, changed, moved, or deleted files are still detected, evidence-validation changes invalidate the index, and the Sync scope window's Full Rescan option bypasses it for the selected system or organization. Current-location refresh, stale-reference retry, Document Renamer candidate discovery, Reconciliation discovery, Evidence Audit discovery, and inspection-inventory discovery read path/filename/size/modified metadata only; only the selected or relevant targets are opened afterward.
...
- Includes a review-first Document Renamer that first discovers candidates from metadata, then reads only those local PDFs, matches existing users, and uses each PDF's immediate containing-folder name as the authoritative organization in the proposed canonical filename. Root-level PDFs use the system organization. A complete valid filename date is reused without requiring a duplicate labeled PDF date. Legacy `Responsibilities` and non-DTA `Course` wording normalize to Privileged User Training; DTA `Course` or `Responsibilities` wording normalizes to DTA Training; and `Awareness` wording normalizes to DoD Cyber Cert. The original PDF can be previewed, and an unchanged SHA-256 is verified after every approved rename. Image-only scans remain available for manual entry and are never guessed.
